...
def parse_args():
    ...
    # 可选参数
    parser.add_argument('--probability_learning_rate', type=float, default=10, help='Probability learning rate')
    ...
    parser.add_argument('--clip_min', type=float, default=0.05, help='Minimum clip value')
    parser.add_argument('--clip_max', type=float, default=0.95, help='Maximum clip value')
...
def test_model(model, dataloader, device='cpu', model_type="stock",base_probs= None,threshold = None):
    model.to(device)
    model.eval()
    all_preds = []
    all_labels = []
    ...
    if threshold is None:
        k = int(all_preds.shape[0] * base_probs[0])
        threshold = kth_largest(all_preds[:,1],k)
